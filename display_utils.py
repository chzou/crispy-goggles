...
def visualize_attack(model, dataset, x_orig, x_adv):
    ...
    orig_pred = model.predict(x_orig[np.newaxis,:])
    adv_pred = model.predict(x_adv[np.newaxis,:])
    orig_txt = dataset.build_text(orig_list)
    adv_txt = dataset.build_text(adv_list)
    orig_html, adv_html = html_render(orig_txt, adv_txt)
    print("Original prediction: ", orig_pred[0][0])
    print('Original Prediction = %s. (Confidence = %0.2f) ' %(('Positive' if orig_pred[0][0] > 0.5 else 'Negative'), np.max(orig_pred)*100.0))
    display(HTML(orig_html))
    print('---------  After attack -------------')
    print('New Prediction = %s. (Confidence = %0.2f) ' %(('Positive' if adv_pred[0][0] > 0.5 else 'Negative'), np.max(adv_pred)*100.0))

    display(HTML(adv_html))

def visualize_attack_erik(model, dataset, x_orig, x_adv):
    x_len = np.sum(np.sign(x_orig))
    orig_list = list(x_orig[:x_len])
    adv_list = list(x_adv[:x_len])
    orig_pred = model.predict(x_orig[np.newaxis,:])
