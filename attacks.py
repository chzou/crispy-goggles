...
class GeneticAtack(object):
    def __init__(self, model, batch_model, 
                 ...
                 n1 = 20, n2 = 5,
                 use_lm=True, use_suffix=False, keras = False):
    ...
    def select_best_replacement(self, pos, x_cur, x_orig, target, replace_list):
        ...
        #print("Words being replaced: ", [w == 0 for w in replace_list])
        new_x_list = [self.do_replace(x_cur, pos, w) if x_orig[pos] != w and w != 0 else x_cur  for w in replace_list ]
        new_x_preds = self.neighbour_model.predict(np.array(new_x_list))[:,0]
        #print("New x_preds: ")
        #print(new_x_preds)
        #print("New x_preds shape: ", new_x_preds.shape)
        #print("original prediction: ", self.model.predict(x_cur[np.newaxis,:])[0,0])
        ...
        #Keep if the label is 1, flip if the label is 0
        new_x_scores = new_x_preds if target == 0 else 1 - new_x_preds

        orig_score = self.model.predict(x_cur[np.newaxis,:])[0,0]
        if target == 1:
            orig_score = 1 - orig_score

        new_x_scores = new_x_scores - orig_score
        #print("Difference with new_x_scores")
        #print(new_x_scores)
        #print("Nex x scores shape: ", new_x_scores.shape)
        ## Eliminate not that clsoe words
        new_x_scores[self.top_n:] = -10000000
            
        if (np.min(new_x_scores) < 0):
            #TODO verify. Looks good if argsort does increased order. TODO check.
            return new_x_list[np.argsort(new_x_scores)[0]]
        return x_cur
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        for i in range(self.max_iters):
            # print(i)
            pop_preds = self.batch_model.predict(np.array(pop))[:,0] #One dimensional
            #print("Pop preds shape: ", pop_preds.shape)
            pop_scores = pop_preds if target == 0 else 1 - pop_preds
            print('\t\t', i, ' -- ', np.max(pop_scores))
            pop_ranks = np.argsort(pop_scores)
            
            top_attack = pop_ranks[0]
            bottom_attack = pop_ranks[-1]
            print("Current lowest: {}".format(pop_scores[top_attack]))
            ...
            print("Pop preds top attack: {}, target: {}, attack successful: {}".format(pop_preds[top_attack], target,
                                                                                       pop_preds[top_attack] > pop_scores[top_attack] < 0.5))                                                                                  
            if pop_scores[top_attack] < 0.5:
                return pop[top_attack]
            #print("Pop top attack: ", pop[top_attack])
...
class PerturbSentBaseline(object):
    def __init__(self, sess, model, batch_model, dataset, dist_mat,
                 lm, n1=20, n2=5, use_lm=True, keras = False):
        self.sess = sess
        self.model = model
        self.batch_model = batch_model
        self.dataset = dataset
